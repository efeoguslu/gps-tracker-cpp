    ...
    while (true) {
        ...
        if (result > 0) {
            ...
            if (n > 0) {
                ...
                // Check if the sentence is valid and log it
                if (!parsedSentence.empty() && parsedSentence[0] == "$GPRMC") {
                    std::string validity = parsedSentence[2];
                    std::string speed = parsedSentence.size() > 7? parsedSentence[7] : ""; // Check if speed exists
                    ...
                    if (validity == "A") {
                        // Turn on the LED
                        // digitalWrite(gpsLedPin, LOW);
                        std::cout << "LED ON test" << std::endl;
                    } 
                    
                    else if (validity == "V") {
                        // Turn off the LED
                        // digitalWrite(gpsLedPin, HIGH);
                        std::cout << "LED OFF test" << std::endl;
                    }
                }
            }
        } else if (result == 0) {
            ...
            // An error occurred
            // std::cerr << "Error in select(): " << std::perror(errno) << std::endl;
